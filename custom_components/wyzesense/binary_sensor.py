    ...
    def on_event(ws, event):
        if event.Type == 'state':
            ...
            data = {
                ...
                ATTR_DEVICE_CLASS: sensor_type.lower(),
                "timestamp": event.Timestamp.isoformat(),  # Store timestamp as a custom attribute
...
class WyzeSensor(BinarySensorEntity, RestoreEntity):
    ...
    def extra_state_attributes(self):
        ...
        # Include the timestamp as an extra attribute if available
        attributes["timestamp"] = self._data.get("timestamp")
        return attributes
